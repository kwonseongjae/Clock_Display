    ...
    try:
        while True:
            if not check_process_running(process_name):
                ...
                program_path = source_path + "/watchdog_reverse.exe"
...
def run():
    process_name = "watchdog.exe"  # 실행 될 프로세스
...
def main(display_number):
    ...
    try:
        while True:
            ...
            # 텍스트 그리기
            draw_text(screen, current_time, font, text_color , 875, 540)
...
if __name__ == "__main__":
    display_number = 1 # 첫 번째 디스플레이는 1, 두 번째 디스플레이는 0, ...
    main(display_number)
    run()
